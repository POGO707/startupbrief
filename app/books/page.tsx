...
import { books, businessArticles } from "@/lib/data";

export const metadata: Metadata = {
  title: "Books | Startup Brief",
  description: "The latest in books.",
};
    ...
    <>
      ...
      <main id="main-content" style={{ minHeight: "100vh" }}>
        ...
        <div className="section" style={{ paddingBottom: 0 }}>
          <div className="container">
            <div className="section-header">
              <h1 className="section-header-title">BOOKS</h1>
            </div>
          </div>
        </div>
                ...
                <Link href={"#"} className="hero-main-link">
                  <div className="hero-main-image img-hover">
                    <Image
                      ...
                      alt={(featured as any)?.title || "Books"}
                      fill
                  ...
                  <div className="hero-main-content">
                    <span className="badge">{(featured as any)?.category || "Books"}</span>
                    <h2 className="hero-main-title link-headline">{(featured as any)?.title || "Featured Books Story"}</h2>
                  ...
                  {(typeof latestNews !== 'undefined' ? latestNews : []).map((item, i) => (
                    <article key={i} style={{ display: "grid", gridTemplateColumns: "32px 1fr", gap: 16, alignItems: "start", padding: "16px 0", borderBottom: "1px solid var(--color-border)" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column" }}>
                        <Link href="#" className="badge" style={{ marginBottom: 6, alignSelf: "flex-start" }}>{(item as any).category || "Books"}</Link>
        ...
        <section className="section editorial-border-top">
          <div className="container">
            <div className="section-header">
              <h2 className="section-header-title">LATEST IN BOOKS</h2>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 32 }}>
              {/* RENDER GRID ITEMS BASED ON ROUTE DATA */}
                ...
                {gridBooks.map((item, i) => (
                  <article key={i} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    <Link href="#">
                      <div className="img-hover" style={{ position: "relative", width: "100%", aspectRatio: "3/4", border: "1px solid var(--color-border)" }}>
                        <Image src={item.cover} alt={item.title} fill sizes="(max-width: 768px) 100vw, 33vw" style={{ objectFit: "cover" }} />
                      </div>
                    </Link>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "flex-start" }}>
                      ...
                      <p style={{ fontFamily: "var(--font-ui)", fontSize: 14, color: "var(--color-secondary)", lineHeight: 1.5 }}>
