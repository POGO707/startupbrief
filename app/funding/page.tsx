...
import { fundingRounds, startupArticles, heroSideArticles } from "@/lib/data";

export const metadata: Metadata = {
  title: "Funding | Startup Brief",
  description: "The latest in funding.",
};
...
export default function FundingPage() {
  ...
  featured = startupArticles[1];
  rounds = fundingRounds.slice(0, 4);
  latestNews = heroSideArticles.slice(0, 4);
    ...
    <>
      ...
      <main id="main-content" style={{ minHeight: "100vh" }}>
        ...
        <div className="section" style={{ paddingBottom: 0 }}>
          <div className="container">
            <div className="section-header">
              <h1 className="section-header-title">FUNDING</h1>
            </div>
          </div>
        </div>
                ...
                <Link href={"#"} className="hero-main-link">
                  <div className="hero-main-image img-hover">
                    <Image
                      ...
                      alt={(featured as any)?.title || "Funding"}
                      fill
                  ...
                  <div className="hero-main-content">
                    <span className="badge">{(featured as any)?.category || "Funding"}</span>
                    <h2 className="hero-main-title link-headline">{(featured as any)?.title || "Featured Funding Story"}</h2>
                  ...
                  {(typeof latestNews !== 'undefined' ? latestNews : []).map((item, i) => (
                    <article key={i} style={{ display: "grid", gridTemplateColumns: "32px 1fr", gap: 16, alignItems: "start", padding: "16px 0", borderBottom: "1px solid var(--color-border)" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column" }}>
                        <Link href="#" className="badge" style={{ marginBottom: 6, alignSelf: "flex-start" }}>{(item as any).category || "Funding"}</Link>
        ...
        <section className="section editorial-border-top">
          <div className="container">
            <div className="section-header">
              <h2 className="section-header-title">LATEST IN FUNDING</h2>
            </div>
              ...
              {/* RENDER GRID ITEMS BASED ON ROUTE DATA */}
                ...
                {rounds.map((item, i) => (
                  <article key={i} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: 12, alignItems: "flex-start", padding: 24, border: "1px solid var(--color-border)", height: "100%" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", width: "100%", alignItems: "center" }}>
                        <Image src={item.logo} alt={item.startup} width={40} height={40} style={{ borderRadius: 8 }} />
                        <span style={{ fontFamily: "var(--font-ui)", fontSize: 24, fontWeight: 700 }}>{item.amount}</span>
                      </div>
                      <h3 style={{ fontFamily: "var(--font-headline)", fontSize: 22, fontWeight: 500, lineHeight: 1.2 }}>
                        {item.startup}
                      </h3>
                      ...
                      <div className="article-meta">
                        <span className="meta-text">{item.date} • {item.country}</span>
                      </div>
                    </div>
                  </article>
                ))}
